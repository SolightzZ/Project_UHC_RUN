...
class UHCGame {
  ...
  static async broadcastMessage(players, { message, title, subtitle, sound, actionBar }) {
    const commands = [];
  ...
  static async showWarning(player) {
    await player.applyDamage(3, { cause: "void", damagingEntity: null });
    const { x, z } = player.location;
    const dx = Math.abs(x) - this.borderRadius;
    const dz = Math.abs(z) - this.borderRadius;
    if (dx > 0 || dz > 0) {
      const message = `§c✘ อยู่ด้านนอก World Border! §fX: ${Math.floor(
        Math.max(0, dx)
      )} Z: ${Math.floor(Math.max(0, dz))}`;
      player.onScreenDisplay.setActionBar(message);
    }
  }

  static getDistanceToBorder(location) {
    return Math.min(
  ...
  static getMaxDistance({ x, z }) {
    return Math.max(Math.abs(x), Math.abs(z));
  }

  static spawnBorderParticles(player) {
    const { location, dimension } = player;
    const distanceToBorder = this.getDistanceToBorder(location);
    ...
    for (const corner of borderCorners) {
      ...
      for (let xOffset = -particleRange; xOffset < particleRange; xOffset += this.particleSpacing) {
        for (
          ...
          addParticle(particleX, location.y + yOffset, particleZ);
        }
      }
    }
  ...
  static updateScoreboard(player) {
  ...
  static startCountdown() {
    ...
    this.countdownId = system.runInterval(async () => {
      if (timeLeft > 0) {
        await this.broadcastMessage(allPlayers, {
        ...
        if (timeLeft === 115) await this.sendVictory();
        if (timeLeft <= 5)
          await this.broadcastMessage(allPlayers, {
            actionBar: "",
            message: "",
  ...
  static async sendVictory() {
    ...
    await this.broadcastMessage(teamMembers, {
      actionBar: "",
      message: "",
      title: "§g§lVictory",
  ...
  static async broadcastBorderWarning(uhcPlayers) {
    const radius = Math.floor(this.borderRadius);
    if (!this.borderWarnings.has(radius)) return;
    this.borderWarnings.delete(radius);
    if (uhcPlayers.length) {
      await this.broadcastMessage(uhcPlayers, {
        actionBar: "",
  ...
  static runGameLoop() {
    this.checkIntervalId = system.runInterval(async () => {
      ...
      await Promise.all(uhcPlayers.map((p) => this.updateScoreboard(p)));
      this.checkGameState(uhcPlayers);
      await this.broadcastBorderWarning(uhcPlayers);
      this.adjustBorder();
      if (this.tickCount >= 0 && this.tickCount <= 40) {
        UHCManager.Timestart(uhcPlayers);
      }

      for (const player of uhcPlayers) {
        const distanceToBorder = this.getDistanceToBorder(player.location);
        const maxDistance = this.getMaxDistance(player.location);
        if (distanceToBorder <= this.borderProximity && maxDistance <= this.borderRadius) {
          this.spawnBorderParticles(player);
        } else if (maxDistance > this.borderRadius) {
          await this.showWarning(player);
          this.spawnBorderParticles(player);
        }
      }
      ...
      if (this.countdownTimes.has(time) && uhcPlayers.length) {
        ...
        await this.broadcastMessage(players, {
          actionBar: "",
          message: "",
          title: "",
          subtitle: "",
      ...
      this.handleEvents(players);
    }, 20);
  }

  static async handleEvents(players) {
